        ...
        <header className="header">
            <div className="header-content">
                <div className="header-title">
                    <a href="/" className="text-slate-400 hover:text-indigo-600 transition-colors flex items-center" title="回首頁" aria-label="返回首頁">
                ...
                <div className="header-controls">
                    ...
                    <div className="toolbar">
                        <button onClick={copyShareLink} className="toolbar-btn" title="複製分享連結" aria-label={copySuccess ? '已複製分享連結' : '複製分享連結'}>
                        ...
                        <button onClick={exportAsImage} disabled={isExporting} className="toolbar-btn" title="匯出圖片" aria-label="匯出為圖片">
                            <Camera className={`icon-sm ${isExporting ? 'animate-pulse' : ''}`} aria-hidden="true" />
                        </button>
                        <button onClick={exportToCSV} className="toolbar-btn" title="匯出 CSV" aria-label="匯出為 CSV 檔案">
                            <Download className="icon-sm" aria-hidden="true" />
                        </button>
                        <button onClick={printReport} className="toolbar-btn" title="列印" aria-label="列印報表">
